...
class TypeSkeleton:
    def __init__(self, typeString = None, instruction = None):
        self.typestring = typeString
        self.instruction = instruction

    def __iter__(self):
        yield self.typestring
        yield self.instruction

    def apply(self, mem, dp):
        pass


class IO(TypeSkeleton):
    def __init__(self, tp):
        super().__init__('I/O', tp)

    def apply(self, mem, dp):
        if self.instruction == 'READ':
            mem[dp.get()] = ord(sys.stdin.read(1))
        elif self.instruction == 'PRINT':
            print(chr(mem[dp.get()]), end="")


class Postpone(TypeSkeleton):
    def __init__(self, tp):
        super().__init__('POSTPONE', tp)

    def apply(self, mem, dp):
        for offset, delta in self.instruction.moves.items():
            mem[dp.get() + offset] = (mem[dp.get() + offset] + delta + 256) & 255
        for offset, setval in self.instruction.fixed.items():
            mem[dp.get() + offset] = setval & 255
        dp.add(self.instruction.final_increase)


class NPostpone(TypeSkeleton):
    def __init__(self, tp):
        super().__init__('NPOSTPONE', tp)

    def apply(self, mem, dp):
        value = mem[dp.get()]
        if value != 0:
            N = calculate_number_of_steps(value, self.instruction.moves[0])
            if N is None:
                raise Exception("While loop that never finishes!")
            for offset, delta in self.instruction.moves.items():
                mem[dp.get() + offset] = (mem[dp.get() + offset] + (delta + 256) * N) & 255
            for offset, setval in self.instruction.fixed.items():
                mem[dp.get() + offset] = setval & 255


class Scan(TypeSkeleton):
    def __init__(self, tp):
        super().__init__('SCAN', tp)

    def apply(self, mem, dp):
        if self.instruction == 1:
            new_dp = mem.index(0, dp.get())
            dp.set(new_dp)
        elif self.instruction == -1:
            new_dp = mem.rindex(0, 0, dp.get() + 1)
            dp.set(new_dp)
        else:
            while mem[dp.get()] != 0:
                dp.add(self.instruction)


class Node(TypeSkeleton):
    def __init__(self, tp):
        super().__init__('NODE', tp)

    def apply(self, mem, dp):
        # we have a son that's also a tree, execute him if while condition is true
        while mem[dp.get()] > 0:
            mem = optinterpret(self.instruction, mem, dp)
...
class OptimizedBrainfuck:
    ...
    .operations represents the instructions needed. can be 4 types (currently):
        ...
        -NODE: While -> has a OptimizedBrainfuck son
    """
...
def merge_operations(first, second):
    ...
    if not ((type_a == type_b and type_a == 'POSTPONE') or (type_a == 'POSTPONE' and type_b == 'NPOSTPONE')):
        # we can't merge
        return None
    # we check conditions for NPOSTPONE
    if type_b == 'NPOSTPONE':
        if instr_a.final_increase in instr_a.fixed:
            # conditions are met, NPOSTPONE becomes just a postpone
            # now we have to find the number of steps
            value = instr_a.fixed[instr_a.final_increase]
            if instr_b.moves[0] != 255:
                return None
            N = calculate_number_of_steps(value, instr_b.moves[0])
            if N is None:
                return None
            for k, v in instr_b.moves:
                instr_b.moves[k] = (v * N) & 255
        else:
            return None
    # here we will try merging instr_a with instr_b
    typestring = 'POSTPONE'
    instr = OptimizedBrainfuck.PostponedMovement()
    instr.final_increase = instr_a.final_increase + instr_b.final_increase

    # this assures we keep the fixed value from the last one
    instr.fixed = instr_a.fixed
    instr.moves = instr_a.moves
...
def modify_nodes_cancellation(root, opass=None):
    ...
    if opass == 'cancellation':
        ...
        if len(root.operations) == 1 and root.parent is not None:
            ...
            if typescript == 'POSTPONE':
                if instruction.final_increase == 0 and len(instruction.moves) == 1 and 0 in instruction.moves and \
                    ...
                    set_0 = OptimizedBrainfuck.PostponedMovement()
        ...
        if len(root.operations) == 1 and root.parent is not None:
            typescript, instruction = root.operations[0]
            if typescript == 'POSTPONE':
                # we can change it to an n postpone if dp doesn't move and it will reach 0:
    ...
    for i, char in enumerate(c):
        if char == '.' or char == ',' or char == '[' or char == ']':
            if i - 1 > last_char_wrong:
                postponed = OptimizedBrainfuck.PostponedMovement()
                postponed.process_code(c[last_char_wrong + 1:i])
                cur_opt.operations.append(Postpone(postponed))
            last_char_wrong = i
            if char == ',':
                cur_opt.operations.append(IO('READ'))
            elif char == '.':
                cur_opt.operations.append(IO('PRINT'))
            elif char == '[':
                # make new leaf
